...
require_basic_tools() {
  local missing=0
  for cmd in bash find grep sed cut sort wc dirname basename mkdir chmod cat cp mv rm date uname; do
...
install_dependencies_if_requested() {
  ...
  if ! command -v quickemu >/dev/null 2>&1 || ! command -v quickget >/dev/null 2>&1; then
    warn "quickemu/quickget not found after package install."
    warn "Install quickemu separately if your repo does not provide it."
  fi
}
...
# Launch modes:
# safe  = SDL windowed
# spice = SPICE/remote-viewer path
...
CONFIG_FILE="$(cd "$(dirname "${BASH_SOURCE[0]}")"/../config && pwd)/tinyvm.conf"
...
CONFIG_FILE="$(cd "$(dirname "${BASH_SOURCE[0]}")"/../config && pwd)/tinyvm.conf"
LIB_FILE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/tinyvm-lib.sh"
# shellcheck source=/dev/null
source "$CONFIG_FILE"
# shellcheck source=/dev/null
source "$LIB_FILE"
...
list_vms() {
  ...
  while IFS='|' read -r vm_name os_type ram cpu disk conf_path launch_mode; do
    [[ -z "$vm_name" ]] && continue
    printf '%2d) %-24s | OS: %-10s | RAM: %-8s | CPU: %-4s | MODE: %-5s\n' "$i" "$vm_name" "$os_type" "$ram" "$cpu" "$launch_mode"
    i=$((i + 1))
  done < "$VM_INDEX_FILE"
}
...
launch_spice() {
  ...
  echo "Window:  ${width}x${height}"
  echo
  echo "If supported by the guest/host path, SPICE gives you menu-driven USB redirection."
  echo
...
final_message() {
  ...
  echo "  ${BIN_DIR}/tinyvm-patch-existing-vms.sh"
  echo
  echo "If you want apt-based dependency install on a fresh host, rerun with:"
  echo "  AUTO_INSTALL_DEPS=1 ./install-tinyvm-v2.sh"
  echo
  echo "Safe mode = SDL windowed"
  echo "SPICE mode = advanced viewer path"
  echo
}
